<?php if(sizeof($this->errors) == 0) {
	if (isset($this->gear_id) && is_numeric($this->gear_id)) { ?>
		...
		<p>The <strong><?=$this->weapon?></strong> weapon has been added to the database.  Click the following button to move onto the next one or 
		wait <?= ($delay = 3) ?> seconds and we'll automatically reload the form for you.</p>
		
		<form method="get" action="/cheatsheets/gear/weapons/entry">
		<input type="hidden" name="page" value="<?= $this->page ?>">
		<input type="hidden" name="book_id" value="<?= $this->book_id ?>">
		<button id="next"><i class="fa fa-fw fa-angle-double-right"></i> Next Weapon</button>
		</form>
	...
	<?php } elseif (isset($this->weapon) && !empty($this->weapon)) { ?>
	
		<p>The most recent weapon added to the database was <strong><?= $this->weapon ?></strong>.</p>
	...
	<?php } elseif (isset($this->error) && !empty($this->error)) { ?>
		...
		<p>We were unable to save this weapon in the database due to a server error of some kind.  The following is what the database told us:</p>  
	...
	<?php } else { ?>
		...
		Use the form below to enter a weapon into the database.

	<?php }
} else { ?>
	<p>We were unable to save this weapon in the databse.  We've placed your entries in the fields below along with error messages
...
if (!isset($this->gear_id) || !is_numeric($this->gear_id)) { ?>
	...
	<legend><label>Enter Weapon</label></legend>
	<p>Required fields are marked <i class="fa fa=fw fa-star"><span>required</span></i>.</p>
	<ol> 
		<li>
			<fieldset>
			<legend><label class="visuallyhidden">Weapon and Weapon Type</label></legend>
			<ol class="nopad bold-labels">	
				<li class="fleft w50">
					<?= $this->printLabel("gear", self::REQUIRED, "Weapon") ?>
					<input type="text" id="gear" name="gear" value="<?= $this->getValue("gear") ?>" class="w100">
				</li>
				<li class="fleft w50">
					<?= $this->printLabel("gear_category_id", self::REQUIRED, "Weapon Type"); ?>
		...
		<li>
			...
			<p>Different attributes are required for different weapon types.  We'll let you know if you forget one.</p>
			<ol class="nopad columns four_columns w90">
				...
				foreach ($this->attributes as $attribute_id => $attribute) {
					...
					<li>
						...
						<input type="text" id="<?= $id ?>" name="attributes[<?= $attribute_id ?>]" min="0" max="50" value="<?= $this->getValue("attributes", "", $attribute_id)?>" class="w90">
					</li>
				<?php } ?>
			</ol>
			</fieldset>
